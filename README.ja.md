...
- **AI円卓会議：** 専用ウィンドウを開き、指定したトピックについてClaude Code、Codex、Antigravityに複数ラウンドの議論をさせられます。参加者、各自が使うモデル、ラウンド数を自分で選べ、開始前にtoken見積もりが表示されます。画像添付と読み取り専用のプロジェクトフォルダにも対応しており、参加者は実際のファイルを参照できます。
