...
- **詳細HTMLレポート：** 日次・週次のtoken推移、プロジェクトランキング、コストを示す、すぐに共有できるHTML詳細レポートです。コントリビューションヒートマップおよび「Wrapped」サマリーを含む**Year in Review**を搭載しています。ワンクリックで **.html、.csv、または.png画像**として保存でき、完全オフラインで、プロジェクト名のマスキングも任意で可能です。
...
- **AI円卓会議：** 専用ウィンドウを開き、指定したトピックについてClaude Code、Codex、Antigravityに複数ラウンドの議論をさせられます。参加者、各自が使うモデル、ラウンド数を自分で選べ、開始前にtoken見積もりが表示されます。各席にAIタレントマーケットの専門家ペルソナを割り当てられるため、同じCLIが異なる2つの役割として2席に参加することもできます。5種類の討論スタイルから選択でき、参加者の同意や反論に応じて変わる態度集計を確認できるほか、全員が合意した時点で議論を早期終了させることも可能です。画像添付と読み取り専用のプロジェクトフォルダにも対応しており、参加者は実際のファイルを参照できます。
- **AI更新日報：** このメニュー項目をクリックすると、既定のブラウザで公開[ウェブページ](https://aqua5230.github.io/ai-updates/)が開きます。ページは毎日自動更新され、Claude Code、Codex、Antigravityの更新を網羅し、完全な履歴を保持します。審査済みの更新は5言語の平易な要約を、未審査のものは公式原文を表示します。
