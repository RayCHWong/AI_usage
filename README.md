...
第一次打開 usag，如果你還沒「對接」過 Claude Code，popover 會偵測到「找不到狀態檔」，**最下面會多出一顆「立即安裝 hook」按鈕**，按一下就會幫你裝好。然後**完全結束 Claude Code（Cmd+Q）再重新打開一次**，在 usag 視窗按「立即更新」，數字就會跑出來。

如果按鈕沒出現（代表 usag 已經抓到資料了，例如你之前裝過 [token-tracker](https://github.com/stormzhang/token-tracker)），就什麼都不用做。
...
> 用 .app 的話，第一次打開直接點 popover 上的「立即安裝 hook」按鈕就好，不用跑這段。下面是給從原始碼跑 usag 的開發者用的。
